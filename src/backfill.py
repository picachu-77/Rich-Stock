...
def main() -> None:
    ...
    if not todo:
        # 여기서 return 하면 안 됩니다.
        #
        # 전에는 곧장 끝냈는데, 그러면 아래 3번 '등락률 정리' 를 건너뜁니다.
        # 하필 등락률 정리는 시세를 다 받은 뒤에야 돌아가는 마지막 단계라,
        # 그 단계에서 실패하면 다시 실행해도 "이미 전부 받았습니다" 하고
        # 끝나버립니다. 고칠 방법이 없어지는 셈입니다. 실제로 그랬습니다.
        #
        # 받을 날짜가 없으면 아래 반복문은 저절로 0번 돌기 때문에,
        # 그냥 통과시키면 등락률 정리만 다시 하게 됩니다.
        print("\n  시세는 이미 전부 받았습니다. 등락률 정리만 다시 합니다.")
    print()
